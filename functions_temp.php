...
function catch_that_image() {
  ...
  if(empty($first_img)) {
    $first_img = "/wp-content/themes/CALSv1_0/images/animalsciences.jpg";
  }
  return $first_img;
}
	...
	if($echo == 1){
		...
		if (count($rss_items) == 0) {
			...
			foreach ( $rss_items as $item ) { 
				...
				<div class="newsItem <?php foreach ($item->get_categories() as $category)
	{
	 ...
	 if(!$featuredImage) {
    		...
    		<div class="text">
    			...
    			<?php $content = $item->get_description(); 
    			
						echo $content;
					
					 ?>
    			
			<?php //the_content_rss('', FALSE, '', 140); ?>
    			...
    			<div class="hiddendate"><?php echo "-".$item->get_date('Ymd'); ?></div>
    		...
    		<a href="<?php echo $item->get_permalink(); ?>" class="highlight" title="<?php echo 'Posted '.$item->get_date('j F Y | g:i a'); ?>">Read more about <?php echo $item->get_title(); ?><div class="loadingSpinner"><div class="progress"></div></div></a>
 ...
 * @link https://yoast.com/display-latest-tweet/ 
 * @link https://stackoverflow.com/questions/1959062/how-to-add-anchor-tag-to-a-url-from-text-input
 * 
*/
function cals_get_last_tweet(){
	...
	//$url  = "https://twitter.com/statuses/user_timeline/uwmadisoncals.json?count=5";
	$url  = "https://api.twitter.com/1/statuses/user_timeline.json?include_entities=true&include_rts=true&screen_name=uwmadisoncals&count=10";
	if ($tweet['lastcheck'] < ( mktime() - 2700 ) ) {
	  ...
	  if (!is_object($result)) { //check if an WP_error object was returned... if so, display error message below 
		...
		//add anchor tag to urls found in tweet's text (i.e. "https://bit.ly/b54pIs")
		...
		$pattern  = '/\@([a-zA-Z0-9_]+)/';
		$replace  = '<a href="https://twitter.com/'.strtolower('\1').'">@\1</a>';
		$output   = preg_replace($pattern,$replace,$output);  
	 
		//add anchor tag to hashtags found in tweet's text (i.e. "#uw2014")
		$pattern  = '/(^|\s)#(\w+)/';
		$replace  = ' <a href="https://twitter.com/search?q=%23'.strtolower('\2').'">#\2</a>';
		$output   = preg_replace($pattern,$replace,$output);  
	...
	if($date_month == "Jan") {
	  ...
	  if($date_day == "01") {
		...
		echo '<div class="newsItem social twitter">
    		...
    		<div class="additionalContent" style="min-height: 130px;"><img src="https://beta.cals.wisc.edu/wp-content/themes/CALSv1_0/images/twitter-bg-'.rand(1, 3).'.jpg" alt=" " /></div>
    		<a href="https://twitter.com/uwmadisoncals/" title="Follow UWMadisonCALS on Twitter" class="highlight">Follow UWMadisonCALS on Twitter</a>
