...
function twentyeleven_theme_options_init() {
	...
	add_settings_field( 'layout',     __( 'Home Page Layout', 'twentyeleven' ), 'twentyeleven_settings_field_layout',     'theme_options', 'general' );
	//add_settings_field( 'slidertoggle',     __( 'Slider Toggle', 'twentyeleven' ), 'twentyeleven_settings_slider_toggle',     'theme_options', 'general' );
...
function twentyeleven_color_schemes() {
	$color_scheme_options = array(
		'light' => array(
			'value' => 'light',
			'label' => __( 'CALS Black', 'twentyeleven' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/light.png',
			'default_link_color' => 'calsblack',
		),
		'dark' => array(
			'value' => 'dark',
			'label' => __( 'UW Red', 'twentyeleven' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/dark.png',
			'default_link_color' => 'uwred',
		),
	);
...
function twentyeleven_featureslider() {
	$slidertoggle_options = array(
		'featureslideron' => array(
			'value' => 'featureslideron',
			'label' => __( 'Show Feature Slider', 'twentyeleven' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/content.png',
		),
		'featureslideroff' => array(
			'value' => 'featureslideroff',
			'label' => __( 'Hide Feature Slider', 'twentyeleven' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/content-sidebar.png',
		),
		
	);

	return apply_filters( 'twentyeleven_featureslider', $slidertoggle_options );
}
...
function twentyeleven_settings_slider_toggle() {
	$options = twentyeleven_get_theme_options();
	foreach ( twentyeleven_featureslider() as $slidertoggle ) {
		?>
		<div class="layout image-radio-option slider-toggle">
		<label class="description">
			<input type="radio" name="twentyeleven_theme_options[slider_toggle]" value="<?php echo esc_attr( $slidertoggle['value'] ); ?>" <?php checked( $options['slider_toggle'], $slidertoggle['value'] ); ?> />
			<span>
				<img src="<?php echo esc_url( $slidertoggle['thumbnail'] ); ?>" width="136" height="122" alt="" />
				<?php echo $slidertoggle['label']; ?>
			</span>
		</label>
		</div>
		<?php
...
function twentyeleven_theme_options_validate( $input ) {
	...
	if ( isset( $input['theme_layout'] ) && array_key_exists( $input['theme_layout'], twentyeleven_layouts() ) )
		...
		// Slider Toggle must be in our array of theme layout options
	if ( isset( $input['slider_toggle'] ) && array_key_exists( $input['slider_toggle'], twentyeleven_layouts() ) )
		$output['slider_toggle'] = $input['slider_toggle'];
...
function twentyeleven_slidertoggle_classes( $existing_classes ) {
	$options = twentyeleven_get_theme_options();
	$current_slidertoggle = $options['slider_toggle'];
	

	/*if ( in_array( $current_slidertoggle, array( 'featureslideron', 'featureslideroff' ) ) )
		$classes = array( 'featureslideron' );
	else
		$classes = array( 'featureslideroff' );*/

	if ( 'featureslideron' == $current_slidertoggle )
		$classes[] = 'featureslideron';
	elseif ( 'featureslideroff' == $current_slidertoggle )
		$classes[] = 'featureslideroff';
	else
		$classes[] = $current_slidertoggle;

	$classes = apply_filters( 'twentyeleven_slidertoggle_classes', $classes, $current_slidertoggle );

	return array_merge( $existing_classes, $classes );
}
add_filter( 'body_class', 'twentyeleven_slidertoggle_classes' );
...
function twentyeleven_customize_register( $wp_customize ) {
	...
	// Default Slider
	/*$wp_customize->add_section( 'twentyeleven_slidertoggle', array(
		'title'    => __( 'SliderToggle', 'twentyeleven' ),
		'priority' => 50,
	) );

	$wp_customize->add_setting( 'twentyeleven_theme_options[slider_toggle]', array(
		'type'              => 'option',
		'default'           => $defaults['slider_toggle'],
		'sanitize_callback' => 'sanitize_key',
	) );

	$slidertoggles = twentyeleven_featureslider();
	$choices = array();
	foreach ( $slidertoggles as $slidertoggle ) {
		$choices[$slidertoggle['value']] = $slidertoggle['label'];
	}

	$wp_customize->add_control( 'twentyeleven_theme_options[slider_toggle]', array(
		'section'    => 'twentyeleven_slidertoggle',
		'type'       => 'radio',
		'choices'    => $choices,
	) );
	*/
	...
	$slider_toggles = twentyeleven_featureslider();
	$choices = array();
	foreach ( $slider_toggles as $slider_toggle ) {
		$choices[ $slider_toggle['value'] ] = $slider_toggle['label'];
	}

	$wp_customize->add_control( 'twentyeleven_slider_toggle', array(
		'label'    => __( 'Slider Toggle', 'twentyeleven' ),
		'section'  => 'colors',
		'settings' => 'twentyeleven_theme_options[slider_toggle]',
		'type'     => 'radio',
		'choices'  => $choices,
		'priority' => 5,
	) );
